...
namespace StockPrice.SiteMethods.USA_Sites
{
    public sealed class BetaMarconeUsa
    {
        ...
        public static async void Parsing(DatabaseTotalResults request, List<MainPriceResponse> mainPriceResponsesList)
        {
            ...
            if (redirect != null) //Если попали на редирект
            {
                totalUrlsList.Add(@$"https://beta.marcone.com{redirect}");
            }

            else //Если не попали на редирект - надо сделать анализ выдачи
            {
            ...
            if (totalUrlsList.Count == 0)
            {
                ...
                await ResponseCreator.MakeResponseLog(con: con,
                    mpr: mpr,
                    request: request);
                return; //Если не нашли товаров вовсе.
            }
            ...
            foreach (var totalUrlTaken in totalUrlsList)
            {
                ...
                if (response != null)
                {
                    ...
                    try//Получение description (title)
                    {
                ...
                try //Запрос к информации о доступности товара
                {
                ...
                if (availabilityResponse != null)
                {
                    ...
                    if (availabilityRegex.Any())
                    {
                        ...
                        if (availabilityAllResponse != null) //Перечень всех локаций.
                        {
