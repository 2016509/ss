...
using ReplacesCreatorDb.Methods;
...
namespace ReplacesCreatorDb.AnalogsUpdater
{
    ...
    {
        public class ReplacesTuple
        {
            public string Name { get; set; } = null;
            public List<string> ReplacesList { get; set; } = null;
            public string Base64Photo { get; set; } = null;
        }
        
        public static ReplacesTuple GetReplaces(string search)
        {
            ...
            //Запускаем поток для PartAdventage.com (Упразнен, нет реплейсов)
            ...
            {
                photoUrl = photos_url_list.OrderBy(x => x.Priority).First().URL;
                photo_source = photos_url_list.OrderBy(x => x.Priority).First().Source;

                try
                {
                    total_photo_stream = CustomHttpClass.GetToStream(photoUrl, use_google_ua: false);
                }
                catch
                {
            ...
            if (total_photo_stream != null)
            {
                ...
                try
                {
                    ...
                    {
                        total_photo_stream.CopyTo(memoryStream);
                        var bytes = memoryStream.ToArray();
                        return new ReplacesTuple()
                        {
                            Base64Photo = Convert.ToBase64String(bytes),
                            Name = title,
                            ReplacesList = replaces_list
                        };
                    }
                ...
                {
                    Log.Warning("Получили ошибку при попытке обработать фотографию.");
        ...
        private static void FixComReplaces(string search, List<string> replaces, List<string> tempNames, List<PhotosData> photos_url_list, List<MainSKUData> mainSKU_list)
        {
            ...
            {
                Log.Warning("Error FIX.COM - STAGE 1 - " + search);
            }

        }

        private static void ReliablePartsReplaces(string search, List<string> replaces, List<string> tempNames, List<PhotosData> photos_url_list, List<MainSKUData> mainSKU_list)
        {
            string location = null;

            try
            {
                location = CustomHttpClass.GetToString(@$"https://www.reliableparts.com/catalogsearch/result/?cat=2&q={search}",
                referrer: "https://www.reliableparts.com/",
                acceptencoding: "none");

               /* if (responseSearchString != null)
                {
                    var responseJson = JsonConvert.DeserializeObject<ReliablePartsSearchResponseDTO>(responseSearchString);
                    if (responseJson is null)
                    {
                        Log.Warning($"Error on ReliableParts registered [Not deserialized data]. STAGE - 0    Request: {search}");
                        return;
                    }
            ...
            catch
            {
                ...
                Log.Warning($"Error on ReliableParts registered. STAGE - 0    Request: {search}");
                return;
            }
            ...
            if (location != null)
            {
                ...
                try
                {
                    string total_result = CustomHttpClass.GetToString(@$"{location}",
                ...
                {
                    Log.Warning($"Error on ReliableParts registered. STAGE - 1    Request: {search}");
                    return;
                }
            }
        }

        private static void AppliancePartsHQReplaces(string search, List<string> replaces, List<string> tempNames, List<PhotosData> photos_url_list, List<MainSKUData> mainSKU_list)
        {
            ...
            {
                Log.Warning($"AppliancePartsHQReplaces ERROR - STAGE 0 - {search}");
            }
            ...
            if (location != null)
            {
                ...
                {
                    Log.Warning($"AppliancePartsHQReplaces ERROR - STAGE 1 - {search}");
                }
            }
            else
            {
                ...
                {
                    Log.Warning($"AppliancePartsHQReplaces ERROR - STAGE 2 - {search}");
                }
                    ...
                    foreach (var search_result_block in search_result_regex_block)
                    {
                        ...
                        if (href_regex.Count > 0 && title_regex.Count > 0 && href_regex.Count == title_regex.Count)
                        {
                            ...
                            if (title_list.Contains(search.ToLower()) || title_list.Contains(search.ToUpper()))
                            {
                                ...
                                {
                                    Log.Warning($"AppliancePartsHQReplaces ERROR - STAGE 3 - {search}");
                                }
        ...
        private static void AppliancePartsPros(string search, List<string> replaces, List<string> tempNames, List<PhotosData> photos_url_list, List<MainSKUData> mainSKU_list)
        {
        ...
        private static void EasyAppliancePartsCaNames(string search, List<string> tempNames)
        {
            //(?<=replaces).*?(?=\.) - для реплейсов (вывод через запятую, требутеся трим)
            startlocation:
            string location = null;
            ...
            {
                Log.Warning($"EasyAppliancePartsCaNames ERROR - STAGE 0 - {search}");
            }
            ...
            if (location != null)
            {
                ...
                {
                    Log.Warning($"EasyAppliancePartsCaNames ERROR - STAGE 1 - {search}");
                }
        ...
        private static void PartSelectComNames(string search, List<string> tempNames)
        {
            ...
            {
                Log.Warning($"PartSelectComNames ERROR - STAGE 0 - {search}");
            }
            ...
            if (location != null)
            {
                ...
                {
                    Log.Warning($"PartSelectComNames ERROR - STAGE 1 - {search}");
                }
        ...
        private static void PartSelectCaNames(string search, List<string> tempNames)
        {
            ...
            {
                Log.Warning($"PartSelectCaNames ERROR - STAGE 0 - {search}");
            }
            ...
            if (location != null)
            {
                ...
                {
                    Log.Warning($"PartSelectCaNames ERROR - STAGE 1 - {search}");
                }
        ...
        private static void ApWagnerComNames(string search, List<string> tempNames, List<PhotosData> photos_url_list)
        {
            ...
            {
                Log.Warning($"ApWagnerComNames ERROR - STAGE 0 - {search}");
            }
            ...
            if (location != null)
            {
                ...
                try
                {
                    ...
                    if (title_result_regex.Count > 0 && title_result_regex.First().Value.Trim().Contains(@"USE WPL"))
                    {
                        ...
                        if (href_use_wpl_regex.Count > 0)
                        {
                            ...
                            {
                                Log.Warning($"ApWagnerComNames ERROR - STAGE 2 - {search}");
                            }
                ...
                {
                    Log.Warning($"ApWagnerComNames ERROR - STAGE 1 - {search}");
                }
        ...
        private static void MidbecComReplaces(string search, List<string> replaces, List<MainSKUData> mainSKU_list)
        {
        ...
        private static void PartAdventageReplaces(string search, List<string> replaces, List<PhotosData> photos_url_list, List<MainSKUData> mainSKU_list)
        {
            ...
            {
                Log.Warning($"PartAdventageReplaces ERROR - STAGE 0 - {search}");
            }
            ...
            if (location != null)
            {
                ...
                {
                    Log.Warning($"PartAdventageReplaces ERROR - STAGE 1 - {search}");
                }
        ...
        private static void AmreSupplyComPhotos(string search, List<PhotosData> photos_url_list)
        {
            ...
            {
                Log.Warning($"AmreSupplyComPhotos ERROR - STAGE 0 - {search}");
            }
            ...
            if (location != null)
            {
                ...
                {
                    Log.Warning($"AmreSupplyComPhotos ERROR - STAGE 1 - {search}");
                }
        ...
        private static void WhirlPoolpartsComPhotos(string search, List<PhotosData> photos_url_list)
        {
            ...
            try
            {
                ...
                string response_search = CustomHttpClass.PostToString(url: $@"https://www.whirlpoolparts.com/PartSearch/GetSearchUrlSSL?searchText={search}&searchContext=w1", 
            ...
            {
                Log.Warning($"WhirlPoolpartsComPhotos ERROR - STAGE 0 - {search}");
            }
            ...
            {
                Log.Warning($"WhirlPoolpartsComPhotos ERROR - STAGE 1 - {search}");
            }
            ...
            if (location != null)
            {
                ...
                {
                    Log.Warning($"WhirlPoolpartsComPhotos ERROR - STAGE 2 - {search}");
                }
